...
impl<'a> Cave<'a> {
    ...
    fn is_small(&self) -> bool {
        self.0.chars().next().unwrap().is_ascii_lowercase()
    }
}
...
impl<'a> Path<'a> {
    ...
    fn is_completed(&self) -> bool {
        ...
        self.0.first().unwrap().is_start() && self.0.last().unwrap().is_end()
    }
}
...
impl<'a> System<'a> {
    ...
    fn find_connected<'b>(&'b self, cave: Cave<'a>) -> impl Iterator<Item = Cave<'a>> + 'b {
        self.0.iter().copied().filter_map(move |conn| match conn {
